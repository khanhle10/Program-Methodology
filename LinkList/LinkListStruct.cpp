...
List List::operator=(const List&  rhs)
{
    ...
    ListNode *result = head;
        for(ListNode *orgi = rhs.head->next; orgi != NULL; orgi = orgi->next){
        result->next = new ListNode;
        result = result->next;
        result->item = orgi->item;
        }
        /*
    if(size > 0){
...
ostream & List::operator << (ostream & out, const List & rhs)
{
    List result;
    result.head = rhs.head;
    while(result.head != NULL){
        if(result.head->next == NULL){
            out<<result.head->item;
        }else{
        out << result.head->item << " ";
